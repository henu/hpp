...
inline void Rawmesh::readVersion100(std::istream& strm)
{
	...
	try {
		...
		// Name
		name = deserializeString(strm, 4, false);

		// Vertices
		uint32_t vrts_size = deserializeUInt32(strm, false);
		...
		// Vertexgroups
		uint32_t vgroups_size = deserializeUInt32(strm, false);
		vgroups.reserve(vgroups_size);
		while (vgroups.size() < vgroups_size) {
			std::string vgroup_name = deserializeString(strm, 4, false);
		...
		// Submeshes
		uint32_t submeshes_size = deserializeUInt32(strm, false);
		...
		while (submeshes.size() < submeshes_size) {
			...
			// UV Component counts
			uint32_t uvcompcounts_size = deserializeUInt32(strm, false);
			...
			// Triangles
			uint32_t tris_size = deserializeUInt32(strm, false);
			new_submesh.tris.reserve(tris_size);
			while (new_submesh.tris.size() < tris_size) {
				Tri new_tri;
				// Vertices
				new_tri.vrts[0] = deserializeUInt32(strm, false);
				new_tri.vrts[1] = deserializeUInt32(strm, false);
				new_tri.vrts[2] = deserializeUInt32(strm, false);
			...
			// Quads
			uint32_t quads_size = deserializeUInt32(strm, false);
			new_submesh.quads.reserve(quads_size);
			while (new_submesh.quads.size() < quads_size) {
				Quad new_quad;
				// Vertices
				new_quad.vrts[0] = deserializeUInt32(strm, false);
				new_quad.vrts[1] = deserializeUInt32(strm, false);
				new_quad.vrts[2] = deserializeUInt32(strm, false);
				new_quad.vrts[3] = deserializeUInt32(strm, false);
			...
			// Material
			new_submesh.mat = deserializeString(strm, 4, false);
		...
		// Materials
		uint32_t mats_size = deserializeUInt32(strm, false);
		while (mats.size() < mats_size) {
			mats.push_back(Rawmaterial(strm));
		}
	}
	catch (Exception const& e) {
		throw Exception("Unable to load raw mesh! Reason: " + std::string(e.what()));
	}
	catch (std::runtime_error const& e) {
		throw Exception("Unable to load raw mesh! Reason: " + std::string(e.what()));
	}
	catch ( ... ) {
		throw Exception("Unable to load raw mesh! Unknown reason!");
	}
